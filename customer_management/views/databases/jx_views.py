...
def upload_vat_report(request):
    ...
    if mark:
        ...
        select_only_sql = "select * from report_path where report_time='%s' and report_type='VAT报告';" \
                          % month
        select_only_res = connect_mysql(sql_text=select_only_sql)
...
def select_inventory_report(request):
    ...
    if page_type == "库存报告":
        ...
        select_sku_report_res = connect_mysql1(sql_text=select_sku_report_sql)
        ...
        select_key_res = connect_mysql1(sql_text=select_key_sql, type="dict")
...
def select_many_report(request):
    ...
    select_report_path_res = connect_mysql(sql_text=select_report_path_sql)
