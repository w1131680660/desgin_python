...
def parameter():
    ...
    product_type_data = conf_fun.connect_mysql_operation(product_type_sql, type='dict')  # 产品类别
    product_state_data = conf_fun.connect_mysql_operation(product_state_sql, type='dict')  # 店铺状态
    channel_data, product_type_data, product_state_data = type_change(channel_data, product_type_data,
                                                                      product_state_data)
...
class Product_Information(ViewSetMixin, APIView):
    ...
    def create(self, request):
        ...
        judge_data = conf_fun.connect_mysql_operation(judge_sql)
        if judge_data:
            self.ret['code'] = 500
