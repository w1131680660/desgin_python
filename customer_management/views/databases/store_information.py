...
def getting_store_data(data_1,page, area):
    ...
    if area == 'all':
        ...
        count_data = connect_mysql(count_sql,type='dict')
    else:
        try:
            area_res = area.split(',')
            area_site = []
        ...
        for i in range(len(area_site)):
            ...
            data_res = connect_mysql(sql,type='dict')
            count_data_res = connect_mysql(count_sql,type='dict')
            for j in data_res:
                data.append(j)
...
def  add_parmeter(data):
    ...
    connect_mysql(platform_sql, type='dict')
    connect_mysql(country_sql ,type='dict')
    connect_mysql(state_sql , type='dict')
...
def parameter():
    ...
    country_data = connect_mysql(country_sql, type='dict')  # 国家
    site_date = connect_mysql(site_sql, type='dict') # 站点
    channel_data = connect_mysql(channel_sql, type='dict') # 渠道 = 平台
    type_data = connect_mysql(type_sql, type='dict') # 店铺类别
    store_state_data = connect_mysql(store_state_sql, type='dict') # 店铺状态
