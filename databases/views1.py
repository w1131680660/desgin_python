...
def get_store_country(request):
    ...
    try:
        data = conf_fun.connect_mysql_re(sql)
...
def get_everyday_sku_order_summary(request):
    ...
    try:
        ...
        print('这是报错的%s'%(e))
        return JsonResponse({"code": 500, "msg": "error:" + '该站点国家的数据不存在'})
...
def get_stock_data(request):
    ...
    if _list is not None:
        ...
        if len(_data) > 0:
            ...
            sql1 = "select count(*) from commodity_information as c join product_message as p ON c.product_code=p.product_code where "
            for i in _data:
...
#            print(769,sql[:-5],'----',start,'---',end,type(end))
#            start = if start
            sql = sql[:-5] + " order by c.id limit {0},{1};".format(start, end)
            print(772 ,sql)
            sql1 = sql1[:-5]
        else:
            sql = "SELECT c.product_code,c.spu,c.sku,c.platform,c.country,c.site,c.commodity_price,p.product_name " \
    ...
    # 获取总条数
    print('792\n',sql1)
    res = conf_fun.connect_mysql_operation(sql1)
    print('???????????????????',res[0][0])
    total_num = res[0][0]
    # 查询详细信息
    for i in data:
