...
def get_func(ad_re_dict, country, auto,spu, site):
    ...
    if re_judge_data:
        ...
        update_sql = " UPDATE advertising_adjustment SET  auto_ad_cost = '{0}' ,no_auto_ad_cost  ='{1}', " \
                     " manual_guidance  = '{2}', cost_rate  = '{3}', automatic_guidance ='{4}' , sales  ='{5}'" \
                     " where id ='6'"\
                    .format(auto_ad_cost , no_auto_ad_cost , manual_guidance,cost_rate , automatic_guidance ,sales , id)
        print(update_sql)
        connect_mysql_master(update_sql)
    else:
        inster_sql = " INSERT  IGNORE  INTO advertising_adjustment ( site,  country,  auto,  spu , dates , auto_ad_cost ," \
                     " no_auto_ad_cost , manual_guidance , cost_rate , " \
    ...
    for data_dict in re_auto_data:
        ...
        if len(spu_list) >1:
            ...
            ad_sql_new = " SELECT times," \
                         ...
                         " spu IN ( {2} ) AND asin != '' GROUP BY times ORDER BY times DESC".format(country_change, site_change,spu_str)
            ...
            ad_sql = " SELECT * FROM advertising_report WHERE countries ='{0}' " \
                     " and company ='{1}' and spu ='{2}' and asin !=''".format(country_change, site_change, spu)
