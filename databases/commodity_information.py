...
def serach_commodity(data, area):
    ...
    if 'all' in area:
        sql = "SELECT c.id,c.product_code, c.spu, c.sku , c.country , c.site , c.platform," \
              ...
              " c.begin_sell_date,c.over_sell_date, c.select_link,c.check_time," \
              "c.sku_link,c.product_link," \
              "s.product_name,s.product_type from commodity_information as c " \
              " LEFT JOIN  product_message as s " \
              " on  c.product_code = s.product_code" \
              "  WHERE c.id >0 {0} {2} limit {1},50"
        count_sql = "SELECT COUNT(c.id) as count_id from commodity_information as c  LEFT JOIN " \
                    "  product_message as s " \
                    " on c.product_code = s.product_code" \
                    "  WHERE c.id >0   {0} {2} limit {1},50"
    else:
        try:
        ...
        sql = "SELECT c.id,c.product_code, c.spu, c.sku , c.country , c.site , c.platform," \
              ...
              " LEFT JOIN  product_message as s " \
              " on  c.product_code = s.product_code" \
              "  WHERE c.id >0 {0} {2} ORDER BY s.product_code DESC limit {1},50"
        count_sql = "SELECT COUNT(c.id) as count_id from commodity_information as c  LEFT JOIN " \
                    "  product_message as s " \
                    " on c.product_code = s.product_code" \
                    "  WHERE c.id >0   {0} {2}  ORDER BY s.product_code DESC limit {1},50"
    ...
    if 'all' in area:
        ...
        count_sql = count_sql.format(search_str, 0, where_str)
        print('all\n', sql)
        data = conf_fun.connect_mysql_operation(sql, type='dict')
...
def add_commodity_data(data):
    ...
    conf_fun.connect_mysql_re(sql,type='dict')
    return res
...
def update_store_data(data):
    ...
    print(update_str)
    sql = "UPDATE commodity_information  SET %s WHERE id='%s'" % (update_str, data.get('id'))
    conf_fun.connect_mysql_operation(sql)
    conf_fun.connect_mysql_re(sql)
...
def product_code_re():
    ...
    data = conf_fun.connect_mysql_operation(sql, type='dict')
    data_2 = conf_fun.connect_mysql_operation(product_state_sql, type='dict')
    return data, data_2
...
class Commodity_Information(ViewSetMixin, APIView):
    ...
    def list(self, request):
        ...
        count_sql = "SELECT COUNT(c.id) as count_id from commodity_information as c  LEFT JOIN " \
                    "  product_message as s " \
                    " on c.product_code = s.product_code" \
                    "  WHERE c.id >0   ORDER BY s.product_code DESC"
        try:
            count_data = conf_fun.connect_mysql_operation(count_sql)
    ...
    def create(self, request):
        ...
        if re_data:
            self.ret['code'] = 500
            self.ret['msg'] = '该国家 站点下的sku已经存在 请勿重复新增'
            return Response(self.ret)
        else:
